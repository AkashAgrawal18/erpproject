...
class Invoice_model extends CI_Model
{
    ...
    public function get_edit_stck_transfer($id)
    {
        ...
        $this->db->join('master_batch_tbl batch', 'batch.m_batch_id = stcktrans.stk_trans_batch', 'left')
            ...
            ->join('master_cate_tbl pkg', 'prod.m_pro_brand = pkg.m_cat_id', 'left');
    ...
    public function insert_stock_transfe()
    {
        ...
        $stk_trans_challan = $this->input->post('stk_trans_challan');
        ...
        foreach ($trans_batch as $key => $cau) {
            $s_data = array(
                "stk_trans_challan" => $stk_trans_challan,
            ...
            if (!empty($trans_id[$key])) {
                ...
                $this->update_batch_qty($cau, $new_qty);
                $res = 2;
            } else {
                ...
                $this->update_batch_qty($cau, $trans_qty[$key]);
                $res = 1;
            }
        }
        return $res;
    ...
    public function delete_stock_transfe()
    {
        ...
        if ($dtype == 1) {
            ...
            foreach ($stck_datil as $kry) {
                $this->update_batch_qty($kry->stk_trans_batch, ($kry->stk_trans_qty * (-1)));
            }
            ...
            $this->update_batch_qty($stck_datil->stk_trans_batch, ($stck_datil->stk_trans_qty * (-1)));
    ...
    public function get_store_availble_batch($todate = '', $storeid = '', $prod_id = '')
    {
        $this->db->select('stk_trans_id,m_batch_id,m_batch_number,m_batch_price,m_batch_mrp,m_batch_date,stk_trans_qty,stk_trans_balqty as balance_qty,stk_trans_to,m_batch_pro_id,prod.m_pro_name,pkg.m_cat_name as package_name,size.m_cat_name as size_name')
            ...
            ->join('master_cate_tbl pkg', 'prod.m_pro_pack = pkg.m_cat_id')->join('master_cate_tbl size', 'prod.m_pro_size = size.m_cat_id')->where('m_batch_balqty >', 0);
        if (!empty($prod_id)) {
    ...
    public function get_availble_batch($todate = '', $ware_id = '', $prod_id = '')
    {
        $this->db->select('m_batch_id,m_batch_number,m_batch_quantity,m_batch_price,m_batch_mrp,m_batch_date,m_batch_balqty as balance_qty,m_batch_ware_id,m_batch_pro_id,prod.m_pro_name,prod.m_pro_pic,cate.m_cat_name as category_name,subcate.m_cat_name as subcategory_name,pkg.m_cat_name as package_name,size.m_cat_name as size_name,brand.m_cat_name as brand_name')->join('master_product_tbl prod', 'prod.m_pro_id = master_batch_tbl.m_batch_pro_id', 'left')->join('master_cate_tbl cate', 'prod.m_pro_cate = cate.m_cat_id')->join('master_cate_tbl subcate', 'prod.m_pro_subcate = subcate.m_cat_id')->join('master_cate_tbl pkg', 'prod.m_pro_pack = pkg.m_cat_id')->join('master_cate_tbl size', 'prod.m_pro_size = size.m_cat_id')->join('master_cate_tbl brand', 'prod.m_pro_brand = brand.m_cat_id')->where('m_batch_balqty >', 0);
        if (!empty($prod_id)) {
            $this->db->where('m_batch_pro_id', $prod_id);
        }
        if (!empty($ware_id)) {
            $this->db->where('m_batch_ware_id', $ware_id);
        }
        if (!empty($todate)) {
            $this->db->where('m_batch_date <=', $todate);
        }

        return $this->db->order_by('prod.m_pro_name')->get('master_batch_tbl')->result();
    }

    public function update_stck_qty($stckId, $qty)
    {
        return $this->db->set('stk_trans_balqty', 'stk_trans_balqty - ' . (int)$qty, FALSE)
            ->where('stk_trans_id', $stckId)->update('stock_transfers');
    }

    public function update_batch_qty($batchId, $batchqty)
    {
        return $this->db->set('m_batch_balqty', 'm_batch_balqty - ' . (int)$batchqty, FALSE)
            ->where('m_batch_id', $batchId)->update('master_batch_tbl');
    }
    /* -------- stock transfer working ---------- */
    /* -------- Invoice working ---------- */
    public function get_edit_invoice($id)
    {
        $this->db->select('invoice_items_tbl.*,m_batch_number,m_batch_balqty as balance_qty,prod.m_pro_name,prod.m_pro_pic,pkg.m_cat_name as package_name,size.m_cat_name as size_name');
