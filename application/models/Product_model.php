...
class Product_model extends CI_model
{
	...
	public function insert_batch()
	{
		...
		$check = $this->db->where('m_batch_id !=', $id)->where('m_batch_number', $m_batch_number)->get('master_batch_tbl')->num_rows();
		if (!empty($check)) {
			return 3;
		}
		$s_data = array(
			"m_batch_number" => $m_batch_number,
		...
		$stktrn_data = array(
			"stk_trans_challan" => "Bno-" . $this->input->post('m_batch_number'),
			"stk_trans_date" => $this->input->post('m_batch_date'),
			"stk_trans_from" => 0,
			"stk_trans_to" => $this->input->post('m_batch_ware_id'),
			"stk_trans_remark" => "Batch Created No-" . $this->input->post('m_batch_number'),
			"stk_trans_status" => 1,
			"stk_trans_prod"    => $this->input->post('m_batch_pro_id'),
			"stk_trans_qty"    => $this->input->post('m_batch_quantity'),
			"stk_trans_balqty"    => $this->input->post('m_batch_quantity'),
		);


		if (!empty($id)) {
			$s_data['m_batch_updatedby'] = $this->session->userdata('user_id');
			$s_data['m_batch_updatedon'] = date('Y-m-d H:i');
			$this->db->where('m_batch_id', $id)->update('master_batch_tbl', $s_data);

			$stktrn_data['stk_trans_updatedby'] = $this->session->userdata('user_id');
			$stktrn_data['stk_trans_updatedon'] = date('Y-m-d H:i');
			$this->db->where('stk_trans_batch', $id)->where('stk_trans_from', 0)->update('stock_transfers', $stktrn_data);
			return 2;
		} else {
			$s_data['m_batch_addedby'] = $this->session->userdata('user_id');
			$s_data['m_batch_addedon'] = date('Y-m-d H:i');
			$this->db->insert('master_batch_tbl', $s_data);
			$batch_id = $this->db->insert_id();
			$stktrn_data['stk_trans_batch'] = $batch_id;
			$stktrn_data['stk_trans_addedby'] = $this->session->userdata('user_id');
			$stktrn_data['stk_trans_addedon'] = date('Y-m-d H:i');
			$this->db->insert('stock_transfers', $stktrn_data);
			return 1;
		}
	}
	...
	{
		$this->db->where('stk_trans_batch', $this->input->post('delete_id'));
		$this->db->delete('stock_transfers');
